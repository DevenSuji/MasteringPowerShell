...
Function Add-ExpiryDataToUser {
    ...
    BEGIN {
        ...
        (Get-ADDefaultDomainPasswordPolicy `
            ...
            -ErrorAction Stop).MaxPasswordAge.Days
    ...
    PROCESS {
        ForEach ($user in $inputObject) {
            ...
            if ( ($daysToExpire.Days -eq "0") -and `
                ($daysToExpire.TotalHours -le $timeToMidnight.TotalHours) ) {
                $user | Add-Member -Type NoteProperty -Name UserMessage -Value "today."
            }
            if ( ($daysToExpire.Days -eq "0") -and `
                ($daysToExpire.TotalHours -gt $timeToMidnight.TotalHours) `
                    -or `
                ($daysToExpire.Days -eq "1") -and `
                ($daysToExpire.TotalHours -le $timeToMidnight2.TotalHours) ) {
                $user | Add-Member -Type NoteProperty -Name UserMessage -Value "tomorrow."
            }
            if ( ($daysToExpire.Days -ge "1") -and `
                ($daysToExpire.TotalHours -gt $timeToMidnight2.TotalHours) ) {
...
Get-EnabledNonExpiringUser | Add-ExpiryDataToUser | Where-Object { $_.DaysToExpire –lt 2 } | Send-PasswordExpiryMessageToUser | Export-CSV report.csv
